...
class WSHandler(websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if message == "Waiting":
            ...
            self.write_message(
                json.dumps([cipher.__dict__ for cipher in retrieved]))
        else:
            ...
            tools.write_result(
                res, votes, tools.retrieve_list(WSHandler.election_name))
...
@app.route("/")
def index():
    elecs = [x[0].replace("./elections/", "", 1)
             for x in os.walk("./elections")]
...
def over(election_name):
    ...
    tornado.ioloop.IOLoop.instance().add_callback(
        callback=lambda: WSHandler.ask_to_mix(election_name))
